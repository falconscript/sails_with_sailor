...
function xdRequest(inputURL) {
	// Local "constants"
  /**
   * NOTE: If there is an error "AS remote" in the console, the YQL file must be rehosted.
   * Currently it is used here on pastebin; but it is xdrequest.XML file on xdrequest.googlecode.com
  */
	var remote_table_definition = "http://pastebin.com/raw/sTkm2PTh";
			...
			function (response) {
				if(response.query && response.query.results) {
					...
					// If the HTML is undefined, we got a bad URL
						...
						//for(i in response.query.results.result)
						//		alert(response.query.results.result[i]+"\n"+i);
							// If we got something other than a 200, append it to the results
							newresult = response.query.results.result;
							newresult.error = response.query.diagnostics &&
								response.query.diagnostics.url[1]["http-status-message"];
